...
Both will try to get the source-strings from the `replace_cache`, which may contain modified
versions of the source-strings.

To add a modified source-string to the `replace_cache`, use `put_into_cache`.
"""
# TODO: rename or extract replacestring stuff (not directly about getting source)
...
def get_module_source(module: ModuleType, use_replace_cache: bool = True) -> str:
    ...
    with suppress(AttributeError):
        return module._pd_source  # TODO: rename, is this still needed?
...
def put_into_cache(key, source: str, repl: str, *loc):
    ...
    :param *loc: The location where *repl* is to be inserted (give as *from_line*, *to_line*,
        *from_col*, *to_col*).
    """
    _ast_utils.AST_NODE_CACHE.clear()  # TODO: is it really needed to clear the full cache?
