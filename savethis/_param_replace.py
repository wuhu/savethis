...
def param(name, val, description=None, use_default=True):  # pylint: disable=unused-argument
    ...
    Parameters can be overridden when loading. See also :func:`savethis.load`.

    This function does nothing in itself. Using it marks the position of a parameter in the
    source code.
...
def params(name, *, use_defaults=True, allow_free=False, **kwargs):  # pylint: disable=unused-argument
    """Helper function for marking parameters.

    Parameters can be overridden when loading. See also :func:`savethis.load`.

    This function does nothing in itself. Using it marks the position of a set of parameters in the
    source code.
...
def extract_param_s(source: str) -> dict:
    """Extract :func:`param` calls from *source*.
    ...
    for call in _dumper.Finder(ast.Call).find(ast.parse(source)):
        ...
        if call.args:
            ...
            assignments['position'] = _ast_utils.get_position(source, call.args[1])
        else:
            # value provided as keyword
            assignments['position'] = [_ast_utils.get_position(source, x.value)
...
def change_param(source: str, param_name: str, value: str) -> str:
    ...
    return _source_utils.replace(source, value, *param_dict['position'], one_indexed=True)
...
def change_params(source, param_name, **kwargs):
    ...
    for k, v in kwargs_.items():
        ...
        if k in params_dict['kwargs']:
            source = _source_utils.replace(source, v, *params_dict['positions'][k], one_indexed=True)
        else:
